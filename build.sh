#!/bin/bash
# Build and optionally push the thath/claude-code-bedrock image.
#
# Usage:
...
IMAGE="thath/claude-code-bedrock"
TAG="latest"
PUSH=false
...
Without --push, builds a local image tagged claude-code-bedrock:latest.
EOF
            exit 0 ;;
...
if [[ "$PUSH" == true ]]; then
    ...
    BUILDER="claude-code-bedrock-builder"
...
# ---------------------------------------------------------------------------
    ...
    TAGS=(-t "claude-code-bedrock:latest")
    [[ -n "$TAG" && "$TAG" != "latest" ]] && TAGS+=(-t "claude-code-bedrock:$TAG")

    echo "Building claude-code-bedrock:latest ($(uname -m))..."
    docker build "${TAGS[@]}" "$SCRIPT_DIR"
    echo "Done. Image tagged claude-code-bedrock:latest"
fi
